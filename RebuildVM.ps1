...
$newVMName = $vmname  ## This script can also rename the computer name. Put new VMName here or replace it with the $vmname variable if you are keeping the name the same.
    ...
    ###     - A new NIC will be created in the $newVNetRGName Resource Group in the existing VNet/Subnet specified in $newVNet and $newSubnet
    ###         - NOTE: This script will not create the new VNet/Subnet
    ###     - The existing NIC(s) will be orphaned.
...
if (-not $changeVNet) {
    ...
    write-host "Adding Fresh NIC in $NewVNet to new VM Object" -ForegroundColor Cyan
    
    $Vnet = Get-AzureRmVirtualNetwork -Name $newVnet -ResourceGroupName $newVNetRGName
    $subnet = Get-AzureRmVirtualNetworkSubnetConfig -Name $newSubnet -VirtualNetwork $Vnet

    $newnic = New-AzureRmNetworkInterface -Name $newNICName -ResourceGroupName $rgName -Location $location -SubnetId $subnet.id
